...
export function CreateBounceHouseForm({ onSuccess, onCancel, onFormDataChange, onSubmit, isSubmitting }: CreateBounceHouseFormProps) {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newImageFiles: ImageFile[] = await Promise.all(
      validFiles.map(async (file) => ({
        ...
        isPrimary: selectedFiles.length === 0 // First image is primary by default
      }))
    );
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare the complete form data with proper type conversion
      ...
      // Update parent component with the complete form data
      onFormDataChange(completeFormData);
      
      // Submit the form using the complete data rather than the event
