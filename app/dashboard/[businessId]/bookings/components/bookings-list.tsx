...
export default function BookingsList({ businessId, initialData }: BookingsListProps) {
  ...
  const { data: bookingsData } = useQuery({
    queryKey: ['bookings', businessId, statusFilter, selectedDate],
    queryFn: async () => {
      const url = new URL(`/api/businesses/${businessId}/bookings`, window.location.origin);
      if (statusFilter !== "all") {
        url.searchParams.append("status", statusFilter);
      }
      if (selectedDate) {
        url.searchParams.append("startDate", selectedDate.toISOString());
      }
  ...
  const filteredBookings = (bookingsData.data || []).filter((booking: Booking) => {
    ...
    return (
      ...
      booking.bounceHouse.name.toLowerCase().includes(searchString) ||
      booking.eventAddress.toLowerCase().includes(searchString)
    );
  });
