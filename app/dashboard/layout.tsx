...
function DashboardUI({ children }: { children: React.ReactNode }) {
  ...
  const { notifications, dismiss } = useNotificationsContext(); // Now safely inside the provider
    ...
    <div className={cn("flex h-full flex-col bg-white", className)}>
      ...
      <nav className="flex-1 space-y-1.5 p-4">
        {sidebarItems.map((item) => {
          // Check if businessId is available before creating link
          const href = businessId ? item.href : '#'; // Fallback or handle loading state
          const isActive = businessId ? pathname === item.href : false;
          
          return (
            <Link
              key={item.title} // Use title or a generated key if href is not unique initially
              href={href}
              className={cn(
                ...
                !businessId && "opacity-50 cursor-not-allowed" // Indicate disabled state
              )}
              onClick={(e) => {
                if (!businessId) e.preventDefault(); // Prevent navigation if no businessId
