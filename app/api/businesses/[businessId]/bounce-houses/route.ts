  ...
  try {
    ...
    const result = await withBusinessAuth(
      ...
      async (business) => {
        ...
        // Transform image data to store URLs
        const imageUrls = validatedData.images.map(img => img.url);
        const primaryImageUrl = validatedData.images.find(img => img.isPrimary)?.url || imageUrls[0];

        // Create the bounce house with its features and images
        const bounceHouse = await prisma.bounceHouse.create({
          data: {
